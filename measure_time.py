...
if __name__ == "__main__":
    face_person_ckpt_path = (
        "/data/dataset/iikrasnova/age_gender/pretrained/checkpoint-377.pth.tar"
    )
    ...
    for ckpt_path, input_size, model_name, num_classes in zip(
        [face_person_ckpt_path, face_age_ckpt_path],
        [face_person_input_size, face_input_size],
        model_names,
        [3, 1],
    ):
        ...
        for batch_size in batch_sizes:
            ...
            for _ in range(steps):
                inputs = (
                    torch.randn((batch_size,) + tuple(input_size)).to(device).half()
                )
            create_t1 = time_sync()
            ...
            with torch.no_grad():
                ...
                all_time = 0
                for _ in tqdm.tqdm(
                    range(steps), desc=f"{model_name} batch {batch_size}"
                ):
                    start = time_sync()
                    inputs = (
                        torch.randn((batch_size,) + tuple(input_size)).to(device).half()
                ...
                print(
                    f"Inference {inputs.shape}, steps: {steps}. Mean time taken {time_taken} ms / image"
                )
